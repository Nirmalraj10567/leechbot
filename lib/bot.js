...
eg. 
    ...
    /search piratebay Chernobyl 720pdd
...
function bot(torrent, bot) {
  ...
  bot.on("message", async msg => {
    ...
    try {
      ...
      bot.sendDocument(msg.chat.id,`${process.env.SITE}api/v1/drive/file/${fileName}?id=${driveId}`)
    ...
    bot.onText(/\/sendme(.+)/, async (msg, match) => {
    ...
    bot.sendDocument(from, "./downloads/sample.txt";
   // bot.sendMessage(from,path);
